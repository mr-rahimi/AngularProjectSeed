...
const createNumberMask = function ({
  prefix = dollarSign,
  suffix = emptyString,
  ...
  function numberMask(rawValue = emptyString) {
    ...
    if (
      ...
      return prefix.split(emptyString).concat([digitRegExp.source]).concat(suffix.split(emptyString))
    } else if (
      rawValue === decimalSymbol &&
      allowDecimal
    ) {
      return prefix.split(emptyString).concat(['0', decimalSymbol, digitRegExp.source]).concat(suffix.split(emptyString))
    }
...
export const MaxLengthNumber = function (maxLength) {
  return {
